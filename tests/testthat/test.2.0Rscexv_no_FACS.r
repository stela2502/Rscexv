...
#if ( ! file.exists(file.path("..","..","data","InbuiltDataNOFACS.RData") ) ){
	InbuiltDataNOFACS <- Rscexv( PCR, FACS, use_pass_fail)
#	save(InbuiltDataNOFACS, file=file.path("..","..","data","InbuiltDataNOFACS.RData") )
#
#}else {
#	data(InbuiltDataNOFACS)
#}
## otherwise I should be able to use the inbuilt data - right?

context('createDataObj NO facs')
InbuiltDataNOFACS@outpath= tempdir()

data <- kick.expressed.negContr.samples(InbuiltDataNOFACS, negContrGenes )
...
xls.files <- c("PCR_color_groups_Heatmap.png","PCR_Heatmap.png")
for ( n in xls.files ) {
	try(expect_that( file.exists( file.path( opath, n ) ), is_true()))
}


if ( ! file.exists(file.path("..","..","data","analyzedNOFACS.RData") ) ){
	InbuiltData <- Rscexv( PCR, FACS, use_pass_fail)
	dataNOFACS <- data
	save(dataNOFACS, file=file.path("..","..","data","analyzedNOFACS.RData") )
